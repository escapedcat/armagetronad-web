...
control in the landscape overlay, and the diff says which. Read it as a tree
rather than a picture -- it lists every descendant of `#touch` whose OWN
computed display is not `none`, so the pad's six buttons appear in it even
though `#pad` is `display:none` and none of it is painted. What it does show
directly is what the cascade hides itself: `tz-left`/`tz-right` are absent,
because `.aa-driving` is off at the language menu where the snapshot is taken.

## portrait/ -- 412x915 dpr 3, `web/tools/portrait-boot-gate.steps`

`[DISPLAY] layout=gameboy`, `[L] NEW_ROUND` reached 6 times, no `[EXCEPTION]`,
no stack overflow, no full SDL queue, no 404 but `/favicon.ico`.
...
names the event that fired instead of dropping it:
    ...
    [TOUCH] orientation portrait=true  (sized for portrait: true) via load
    [TOUCH] orientation portrait=false (sized for portrait: true) via resize
    [TOUCH] orientation portrait=false (sized for portrait: true) via media query

## landscape/ -- 915x412 dpr 3, `web/tools/touch-gate.steps`

`[DISPLAY] layout=full` on both boots (the script reloads for T1c),
`[TOUCH] enabled=true`, `[L] NEW_ROUND` 10 times, the only two `[EXCEPTION]`
lines are the script's own positive control at the end, no stack overflow, no
full SDL queue, no 404 but `/favicon.ico`.
