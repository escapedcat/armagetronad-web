...
>
> Annotated at M4 exit. Corrections are inline throughout; this is the index, because a reader who acted on a wrong line needs to find the retraction from the top rather than by luck.
>
> | where | what it said | what is true |
> |---|---|---|
> | Global Constraints | "must still produce EXACTLY 2,488,298 bytes" | **Size alone never detected this class of change.** Task 3 measured an unguarded build at exactly that size with a different md5, delta zero. Corrected in place; quote size AND md5. |
> | Task 3, step 3 | "compile each object at the **same path**" | Wrong reason, right outcome. The `-o` path is irrelevant — the **source basename** perturbs the object. Corrected in place with the measurement. |
> | Architecture | "no C++ at all" for the persistence half | **Held.** `autoPersist` hooks the `ofstream` close and `tConfiguration.cpp` was never touched. The one C++ file M4 added is client-only and outside `$(SRCS)`. |
> | Landmine 1 | the un-awaited populate is the intermittent failure | **Held, and it was the right thing to fear** — but the *obvious* control does not control. Deleting the run dependency left the check passing, because `run()`'s 1 ms yield plus ~280 ms of `initRuntime` is a race IndexedDB happened to win. The real control slows the populate to 3 s. |
> | Landmine 6 | `rViewport.cpp:246` is a latent abort the gate must avoid | **Held.** Every M4 gate route avoids the viewport screen deliberately. Still unfixed; still M5's. |
> | Task 2 | assumed an edit to `src/ui/` or `src/tron/` | **Wrong, and the correction removed risk.** `uMenu.h` already exports `uCallbackMenuLeave`, so the mechanism is a registration from a new client-only TU and the dedicated byte-identity is structural rather than argued. |
> | Task 4, A3 | the counter decrements "only when a live cycle accepted the action" | **Wrong.** `gCycle::Act` has no aliveness guard on its turn arms and `gCycle::Kill` keeps the object, so a press after death still decrements. A3 proves the keycode round trip — delivery, `keymap[]` resolution, the camera declining, the cycle object executing `Turn()` — which is what it was commissioned for, and not "the game steered". |
>
> **The one thing this plan did best:** the landmines were written as *predictions with measurements attached*, and landmine 1 is why Task 1 gated the populate rather than hoping. **The one it did worst:** it inherited `PLAN.md`'s premise that settings are never saved, which recon refuted before Task 1 began — the plan would have had M4 solve a problem that was not the problem. **The recurring defect, for the fourth milestone running:** confident enumerations that turn out wrong. Eight were caught here, including two in a reviewer's own list and two of the controller's — one reached three files. *If a sentence counts things, count them again, and say what basis you are counting on.*
...
- Guard game-source changes: `#ifdef __EMSCRIPTEN__`, or `#if !defined(DEDICATED) && defined(__EMSCRIPTEN__)` where the file **also compiles into the dedicated build**. `web/Makefile` wildcards all six source directories for both targets, so assume any `src/` file does unless proven otherwise. Verify by compiling the object with the same flags to a scratch location and comparing md5 against base. **CORRECTED AT M4 EXIT — this line said "at the same path" and gave the wrong reason.** Measured: two different `-o` paths give **identical** object md5s, so the output path is irrelevant. What perturbs the object is the **source basename** — the same content under a different filename gives a different md5 at an identical size. Keeping the path happened to work because it keeps the basename; anyone who followed the stated reason by copying the source to a scratch *file* would have measured the rename instead of the change. Hold the basename constant. (Task 3's step 3 carried the same error and was corrected there first; this instance survived that pass — which is the enumeration defect in miniature.)
